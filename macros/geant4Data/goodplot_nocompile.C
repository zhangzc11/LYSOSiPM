...
float drawT100(TTree * tree_center, TTree * tree_edge, std::string& inFileName, std::string& plotDir)
{
	...
	TH1F * h100_center = new TH1F("h100_center","h100_center",100,0,0.5);
	TH1F * h100_edge = new TH1F("h100_edge","h100_edge",100,0,0.5);
	...
	TLegend * leg  = new TLegend (0.45,0.77,0.80,0.92);
        ...
        leg->AddEntry(h100_center, "center (x = 0mm)", "l");
        leg->AddEntry(h100_edge, "edge (x = 5mm)", "l");
        leg->Draw();		
...
void draw_average_pulse(TTree * tree_center, TTree * tree_edge, std::string& inFileName, std::string& plotDir, std::string timetag)
{
	...
	TLegend * leg;
	if(timetag == "1") leg = new TLegend(0.6,0.75,0.85,0.9);
	else leg = new TLegend(0.6,0.2,0.85,0.35);
	leg->SetBorderSize(0);
	leg->SetTextSize(0.06);
	leg->SetLineColor(1);
	leg->SetLineStyle(1);
...
void goodplot_nocompile()
{
	...
	string plotDir = "/Users/zhicai/cernbox/TestBeam/geant4/plots/";
	...
	nPhotons_center = draw_nPhotons(tree_center, inFileName_center, plotDir, "nPhotons", 600000.0);
	nPhotons_edge = draw_nPhotons(tree_edge, inFileName_edge, plotDir, "nPhotons", 600000.0);
	float nPhotons_peak_center = draw_nPhotons(tree_center, inFileName_center, plotDir, "int_peak", 100000.0);
	float nPhotons_peak_edge = draw_nPhotons(tree_edge, inFileName_edge, plotDir, "int_peak", 100000.0);
	float nPhotons_amp_center = draw_nPhotons(tree_center, inFileName_center, plotDir, "amp", 15000.0);
	float nPhotons_amp_edge = draw_nPhotons(tree_edge, inFileName_edge, plotDir, "amp", 15000.0);
	float nPhotons_firstbin_center = draw_nPhotons(tree_center, inFileName_center, plotDir, "int_firstbin", 100.0);
	float nPhotons_firstbin_edge = draw_nPhotons(tree_edge, inFileName_edge, plotDir, "int_firstbin", 100.0);
