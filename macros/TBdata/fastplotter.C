...
void fastplotter(const std::string& inFileName)
{
	string ch1 = "15";
	string ch2 = "9";

	mkdir("./DQM", S_IRWXU | S_IRWXG | S_IRWXO);
	mkdir("./DQM/png", S_IRWXU | S_IRWXG | S_IRWXO);
	mkdir("./DQM/C", S_IRWXU | S_IRWXG | S_IRWXO);

	cout<<"plotting run:"<<inFileName<<endl;

	string cut = "amp["+ch1+"]>0.1 && amp["+ch1+"] < 0.42 && amp["+ch2+"]>0.1 && amp["+ch2+"]<0.42 && x1 > 5000 && x1<35000";
	string cut1 = "amp["+ch1+"]>0.02 && x1 > 5000 && x1<35000";
	string cut2 = "amp["+ch2+"]>0.02 && x1 > 5000 && x1<35000";
 	
	gStyle->SetOptStat(0);
	gStyle->SetOptFit(111);

	TFile* inFile = TFile::Open(("RawDataSaver0CMSVMETiming_Run"+inFileName+".root").c_str(),"READ");
	...
	//amplitude
	TH1F * h_ch1E = new TH1F("h_ch1E","h_ch1E",800,0, 2.0);
	TH1F * h_ch2E = new TH1F("h_ch2E","h_ch2E",800,0,2.0);
	...
	float maxX_ch1 = std::max(2.0*maxX_ch1E, highch1E+0.2);
	...
	float maxX_ch2 = std::max(2.0*maxX_ch2E, highch2E+0.2);
	...
	h_ch1E->GetXaxis()->SetTitle("Amplitude [V]");
	...
	myC->SaveAs(("./DQM/"+inFileName+"_amplitude.pdf").c_str());
	myC->SaveAs(("./DQM/png/"+inFileName+"_amplitude.png").c_str());
	myC->SaveAs(("./DQM/C/"+inFileName+"_amplitude.C").c_str());

	// time resolution
	TH1F * h_deltaT = new TH1F("h_deltaT","h_deltaT",8000,-50.0, 50.0);
	tree->Draw(("linearTime15["+ch1+"]-gauspeak["+ch2+"]>>h_deltaT").c_str(), cut.c_str());	
	h_deltaT->SetTitle("");
	...
	myC->SaveAs(("./DQM/"+inFileName+"_deltaT.pdf").c_str());
	myC->SaveAs(("./DQM/png/"+inFileName+"_deltaT.png").c_str());
	myC->SaveAs(("./DQM/C/"+inFileName+"_deltaT.C").c_str());
